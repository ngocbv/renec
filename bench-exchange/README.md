...
```bash
$ RUST_LOG=renec_bench_exchange=info cargo test --release -- --nocapture test_exchange_local_cluster
```

To also see the cluster messages:

```bash
$ RUST_LOG=renec_bench_exchange=info,solana=info cargo test --release -- --nocapture test_exchange_local_cluster
```
