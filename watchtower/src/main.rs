...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `renec-watchtower`:
        ...
        define environment variable before running `renec-watchtower`:
    ...
    loop {
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(&format!("renec-watchtower: {}", all_clear_msg));
            }
