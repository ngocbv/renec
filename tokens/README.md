...
```bash
renec-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
renec-tokens transaction-log --output-path transactions.csv
```
...
```bash
renec-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
renec-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
renec-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
renec-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
renec-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `renec-tokens balances`.
...
```bash
renec-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
