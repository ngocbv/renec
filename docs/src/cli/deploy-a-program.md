...
```bash
renec program close <BUFFER_ADDRESS>
```

To close a single account and specify a different authority than the default:

```bash
renec program close <BUFFER_ADDRESS> --buffer-authority <KEYPAIR_FILEPATH>
```

To close a single account and specify a different recipient than the default:

```bash
renec program close <BUFFER_ADDRESS> --recipient <RECIPIENT_ADDRESS>
```
